...
class setup_release_branches:
    ...
    def get_sprint_start_dates(self):
        ...
        sprintStartDates = {row.cells[1].value: datetime.strptime(row.cells[3].value, '%Y-%m-%dT%H:%M:%S').date() for row in sheet.rows if 'sprint starts' in row.cells[1].value.lower() or 'sprintstarts' in row.cells[1].value.lower() or 'sprint-starts' in row.cells[1].value.lower() }
